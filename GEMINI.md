...
| Web Automation    | Playwright (TS/Java), Selenium WebDriver (Java), Cypress (TS) |
| Mobile Automation | Appium (Java)                                   |
| API Automation    | REST Assured                                    |
| Test Framework    | TestNG, Playwright Test, Cypress Test Runner    |
...
- [Quy tắc Cypress](.agent/rules/cypress_rules.md) — Locator priority, cy.session(), cy.intercept(), NO cy.wait(ms)
...
| `cypress_script_generator`    | Sinh Cypress automation scripts (POM + Custom Commands + cy.session + auto-heal)         |
...
| Selenium   | `WebDriverWait` + `ExpectedConditions`                           |
| Appium     | `WebDriverWait` + custom conditions                                |
| Cypress    | `.should('be.visible')`, `cy.intercept()` + `cy.wait('@alias')`  |
...
| `/generate_automation_framework`        | Thiết kế automation framework (Playwright / Selenium / Appium) |
| `/generate_automation_framework_cypress` | Thiết kế và scaffold Cypress framework (v13+ + TypeScript + POM + CI/CD) |
| `/generate_automation_from_testcases_cypress` | Chuyển manual test cases → Cypress automation scripts |
